      ...
      <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fit transitions-theme ${ifExists ? 'h-70 lg:h-65 md:h-60 sm:h-59 xsm:h-55 animate-none hover:scale-110' : 'h-73 lg:h-64 md:h-60 sm:h-59 xsm:h-55 animate-none hover:scale-110'}`}
          />
        </div>
      </div>
   </>
